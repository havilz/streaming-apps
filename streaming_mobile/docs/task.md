...
### Task 21. Implementasi Autentikasi & Lencana Profil Dinamis (Frontend)
- [x] Buat widget molecule `ProfileBadge` di `lib/shared/molecules/profile_badge.dart` yang mendukung inisial huruf untuk pengguna login & default icon untuk tamu.
- [x] Hapus folder fitur profil `lib/features/profile` lama beserta seluruh berkasnya.
- [x] Hubungkan `ProfileBadge` dan teks status/username dinamis (atau tombol datar "Sign In / Sign Up") ke dalam `MenuModal` menggantikan header `SV MENU`.
- [x] Buat modul fitur baru `lib/features/auth` dengan `auth_provider.dart` (StateNotifier Riverpod) dan `auth_screen.dart` (layar formulir login/daftar).
- [x] Daftarkan rute baru `/auth` di `app_router.dart` dan ekspor modul di barrel file `features.dart`.
- [x] Jalankan static analysis `flutter analyze` dan pastikan seluruh test suite lolos.
